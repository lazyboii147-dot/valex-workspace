# ============================================================================
# FILE: Stage-Assets.ps1
# DESCRIPTION: Upstream Staging Provider & Automated Ingestion Trigger
# AUTHORIZATION: Station 90247-Alpha Pipeline Feed
# ============================================================================

# 1. Define Feed Origins and Pipeline Drop Zones
$ScanOriginDir = "C:\Users\enriq\VALEX\01_INTAKE"
$PipelineDrop  = "C:\Users\enriq"
$EnginePath    = "C:\Users\enriq\VALEX\Process-Intake-Final.ps1"

Write-Host "[*] Scanning staging origin: $ScanOriginDir" -ForegroundColor Cyan
...
Write-Host "[!] Found $($PendingItems.Count) asset(s) ready to be pushed to the pipeline." -ForegroundColor Yellow

# 4. Format, Rename, and Inject Assets into the Pattern Zone
foreach ($Item in $PendingItems) {
    # Check if name already starts with VALEX to preserve indexing tags
    if ($Item.Name -like "VALEX*") {
        $StagedName = $Item.Name
    } else {
        $StagedName = "VALEX_" + $Item.Name
    }
